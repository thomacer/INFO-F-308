...
Les algorithmes de multiprécision sont aujourd'hui indispensable même si
beaucoup d'applications de la vie quotidienne ne sont pas contraintes par les
architectures actuelles. C'est un tout autre discour pour la sécurité
...
informatique par exemple est de plus en plus utilisé et importante pour
les utilisateurs d'ordinateur, il suffit de visiter un site internet pour
qu'on execute sur nos ordinateurs des algorithmes qui se basent sur les
...
sur lesquels elles pourraient être executées.\\

Il est important de comprendre suite à la lecture de ce papier qu'il n'existe
jamais une solution universelle quand il s'agit de résoudre des opérations en
...
De plus j'ai ici choisis de ne pas aborder des algorithmes qui ont leur place
...
la librairie \emph{GMP}, ou l'algorithme de \emph{Schönhage et Strassen} qui
sont asymptotiquement plus efficace que les algorithmes de \emph{Toom}.\\

J'ai aussi choisis de ne pas comparer \emph{GMP} avec d'autres librairies
faisant de l'arithmétique multiprécision comme par exemple \emph{GNU MPFR} ou
\emph{OpenSSL}. Il aurait été interessant de faire une évaluation des
performances selon l'opération de chacune d'elle pour savoir qu'elles sont les
applications où une librairies est plus intéressante qu'une autre.\\

Si le lecteur veut s'informer plus sur les algorithmes abordé ici, il est
particulièrement interessant de lire les implémentations des algorithmes par
\emph{GMP} ceux-ci on y apprend plus sur les optimisations de performances
operées par les créateurs de la librairie.
